...
export function buildInvoicePdfIntoDoc(
  ...
  if (notesTrimmed) {
    ...
    for (const textLine of noteLines) {
      ensureRoomForBlock(BODY_LINE_H_MM)
      doc.text(textLine, PAGE_MARGIN_MM, cursorYMm)
      moveDown(BODY_LINE_H_MM * 0.92)
    }
    doc.setTextColor(0, 0, 0)
    moveDown(SECTION_TOP_MARGIN_MM * 0.5)
  }
  ...
  if (hasPaymentDetails(payment)) {
    ensureRoomForBlock(14)
    moveDown(1.1)
    const paySectionTop = cursorYMm
    doc.setDrawColor(228, 228, 231)
    doc.line(PAGE_MARGIN_MM, paySectionTop - 0.5, contentRightMm, paySectionTop - 0.5)
    moveDown(2)
    ...
    if (payment.bankName.trim() !== '') {
      payPairs.push({ label: p.bank, value: payment.bankName.trim() })
    }
    if (payment.registrationNumber.trim() !== '') {
      payPairs.push({ label: p.registrationNumber, value: payment.registrationNumber.trim() })
    }
    if (payment.accountNumber.trim() !== '') {
      payPairs.push({ label: p.accountNumber, value: payment.accountNumber.trim() })
    }
    if (payment.accountHolder.trim() !== '') {
      payPairs.push({ label: p.accountHolder, value: payment.accountHolder.trim() })
    }
    if (payment.iban.trim() !== '') {
      payPairs.push({ label: p.iban, value: payment.iban.trim(), mono: true })
    }
    if (payment.bicOrSwift.trim() !== '') {
      payPairs.push({ label: p.bicSwift, value: payment.bicOrSwift.trim(), mono: true })
    }
    if (payment.paymentReference.trim() !== '') {
      payPairs.push({ label: p.paymentRef, value: payment.paymentReference.trim() })
    }
    cursorYMm = writePaymentTwoColumnRows(doc, cursorYMm, pageHeightMm, PAGE_MARGIN_MM, contentWidthMm, payPairs)
    moveDown(4)
  }
