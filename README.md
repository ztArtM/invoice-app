...
**Vercel (recommended for this repo):** connect the GitHub repo; Vercel runs `npm run build` and serves the static app from `dist/` (`vercel.json` rewrites unknown paths to `index.html` for the SPA).

You can deploy the `dist/` folder to any static host (Netlify, Cloudflare Pages, GitHub Pages, etc.).
